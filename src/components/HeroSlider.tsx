...
const HERO_SLUGS = [
  ...
  { slug: "new-kia-sorento", tagline: "SUV mạnh mẽ, sang trọng", badge: "SUV 7 chỗ", bg: "from-[#1a0a0e] to-[#3d1520]" },
  { slug: "new-kia-seltos", tagline: "Crossover năng động", badge: "Bán chạy số 1", bg: "from-[#0a1628] to-[#152d4a]" },
  { slug: "kia-sportage", tagline: "Phong cách táo bạo", badge: "SUV Cao cấp", bg: "from-[#1a1000] to-[#3d2800]" },
];

export default function HeroSlider({ cars }: { cars: Car[] }) {
  const [current, setCurrent] = useState(0);
  const { openModal } = useModal();

  const slides = HERO_SLUGS.map((s) => ({
    ...s,
    car: cars.find((c) => c.slug === s.slug) ?? cars[0],
  })).filter((s) => s.car);
